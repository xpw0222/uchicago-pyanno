...
# aAb
		...
		summa2 = (1-alphas[0])*x1 + (1-alphas[1])*x2 + (1-alphas[2])*x3 + summa1*x4
...
def likeA8(x,arguments):
    ...
    for i in range(8):
		...
		l -= likeA(xx,alphas,omegas,counts[:,i],usepriors,estimatealphas,dim)
...
# prior on *thetas*
	...
	for i in range(dim):
		for j in range(dim):
			for k in range(dim):
				like += counts[ii]*log(expPat(al,omegas,x[0],x[1],x[2],i,j,k))
				ii += 1
	del al
	return like
#----------------------------------------------------------
...
def format_posteriors(mat, alphas, dimension, thetas, gam, modelnumber):
    ...
    for row in mat:
        ...
        if modelnumber == 1:
            ...
            p = computePosteriorBt(vijk[0],vijk[1],vijk[2],tijk[0],tijk[1],tijk[2],gam,dimension)  
        posteriors[i,:] = p 
        i+=1
...
def check_data(filename,optional_labels,report):
    ...
    if cmp(report,'Nothing')!=0:
        print string_wrap(num2comma(n)+" lines in file "+filename+'...', 1)
...
def optimum_jump(likelihood,x0,arguments,
                 x_upper, x_lower,
                 evaluation_jumps,recomputing_cycle,targetreject,Delta,report):
...
def adjustJump(dx,Rej,targetreject,Delta):
    ...
    return dx,check
#=================================================================================================
def sample_distribution(likelihood, x0, arguments, dx, Metropolis_jumps, x_lower, x_upper, report):
...
def analyse_parameter_distribution(values, confidence, nbs):
    ...
    nn,bins=histogram(values, bins=nbs, range=None, normed=False, weights=None, new=None)
...
def plot_modelA(values,confidence,dpidpi,numbins,x_best):
    ...
    delta = 0
    leg0 = ['$A_1$','$A_2$','$A_3$','$A_4$','$A_5$','$A_6$','$A_7$','$A_8$']
    pyplot.title('Annotator-specific accuracy ($\\theta_i$ $\\rightarrow$ $A_i$)')

    increment_delta=False

    for i in range(8):
        v_tmp = values[:,i]
        x,y,xl,xr=analyse_parameter_distribution(v_tmp,confidence,numbins)
        x_left[i]=xl
        x_right[i]=xr
        col = cm.jet(i/8.)
        step(x,y+delta,linewidth=3,color=col,linestyle='-',where='mid',label=leg0[i])
        ax1.plot([xl,xl],[0+delta,get_y(xl,x,y)+delta],[xr,xr],[0+delta,get_y(xr,x,y)+delta],color=col,linestyle='-',label='_nolegend_')
        ax1.plot([x_best[i],x_best[i]],
             [0,get_y(x_best[i],x,y)+delta],linewidth=1,color=col,linestyle='--',label='_nolegend_')
    ...
    leg = ('$\\alpha_1=\\alpha_2=\\alpha_3$','$\\alpha_4$','$\\alpha_5=\\alpha_6=\\alpha_7$')
    ...
    if len(x_best)>8:
        ...
        for i in range(8,11):
            ...
            x_left[i]=xl
            x_right[i]=xr
            ax.step(x,y+delta,linewidth=3,color=col,linestyle='-',where='mid',label=leg[i-8])
            ax.plot([xl,xl],[0+delta,get_y(xl,x,y)+delta],[xr,xr],[0+delta,get_y(xr,x,y)+delta],color=col,linestyle='-',label='_nolegend_')
...
class ABmodelGUI(HasTraits):
   ...
   # look_at_raw_data = Button('Show raw data')
    ...
    def _look_at_raw_data_fired(self):
        data_view = View(Group(Item(name='raw_annotations',label='Units of annotation'),
                           show_border=True),
                           title='Annotators',
        ...
        mat,dim,values,imap,originaldata,annotators,codes,n = check_data(self.input_data,self.optional_labels,self.report)
...
#============================================================================
    def run_estimation(self, model, 
        ...
        if cmp(report,'Nothing')!=0:
            ...
            print 'Estimate variances: '+string_wrap(str(estimate_variances),4)
            print 'Metropolis jumps: '+string_wrap(str(Metropolis_jumps),4)
            print 'Report: '+string_wrap(str(report),4)
            print 'Target rejection rate (%): '+string_wrap(str(target_reject_rate),4)
    ...
    #------ prepare data ---------------------------------------------------------------------------------
        mat,dim,values,imap,originaldata,annotators,codes,n = check_data(filename,optional_labels,report)
...
# model Bt
        ...
        fileout = form_filename(filename,modelname+'_'+str(numberOfRuns)+'_posteriors.txt')
        ...
        for j in range(numberOfRuns):
            ...
            if modelnumber == 1:
                ...
                x_best = scipy.optimize.fmin(likeA8, \
					x0, args=arguments, \
					xtol=1e-8, ftol=1e-8, disp=False, maxiter=1e+10, maxfun=1e+30)
                ...
                x_best = scipy.optimize.fmin(likeBt8, \
                    x0, args=arguments, \
                    xtol=1e-8, ftol=1e-8, \
                    disp=False, maxiter=1e+10, maxfun=1e+30)
            ...
            #------------------------------------
            fs,xs,ts = load_save_parameters(filename,modelname,FF[j],x_best,report)
            #------------------------------------

            if modelnumber == 1:
                ffile.write(' Thetas: '+array_format(Res[j,0:8],'%4.3f ')+'\n')
                if estimatealphas==1:
                    ffile.write(' Alphas: '+array_format(Res[j,8:11],'%4.3f ')+'\n')

                if cmp(report,'Nothing')!=0:
                    print_wrap(' Thetas: '+array_format(Res[j,0:8],'%4.3f '))
                    if estimatealphas==1:
                        print_wrap(' Alphas: '+array_format(Res[j,8:11],'%4.3f '))
            else:
                ffile.write(' Gammas: '+array_format(Res[j,0:dim-1],'%4.3f ')+'\n')
                ffile.write(' Thetas: '+array_format(Res[j,dim-1:dim-2+9],'%4.3f ')+'\n')
                if cmp(report,'Nothing')!=0:
                    print_wrap(' Gammas: '+array_format(Res[j,0:dim-1],'%4.3f '))
                    print_wrap(' Thetas: '+array_format(Res[j,dim-1:dim-2+9],'%4.3f '))
        ...
        #--------------------------------------------------------------------------
        # --------------- save mea-data -------------------------------------------
        save_metadata(filename, originaldata, annotators, codes, n, omegas, report)
        #--------------------------------------------------------------------------
#-------------------------
# define best estimates        
        ...
        fs,xs,ts = load_save_parameters(filename,modelname,f_best,x_best,report)
        x0[:] = xs[:]
        x_best[:] = xs[:]
...
# estimate variances and credible intervals of parameters
        ...
        if estimate_variances == True:
            if cmp(report,'Nothing')!=0:
                print string_wrap('**Preparing to compute credible intervals**',4)
            ...
            if cmp(report,'Nothing')!=0:
                print string_wrap('Optimizing jump (to get closer to the target rejection)...',4)
            ...
            Samples = sample_distribution(likelihood, x0, arguments, 
                                          dx, Metropolis_jumps, x_lower, x_upper, report)
            print "Save samples!!!"
            fi = filename.split('.')
            filearray = fi[0]+modelname+'_'+str(numberOfRuns)+'_MCMC.txt'
            ...
            dpidpi = 150
            if modelnumber == 1:
                x_left,x_right = plot_modelA(Samples,Level,dpidpi,numbins,x0)
                for i in range(len(x_best)):
                    print str(x_best[i]), ', CI : [', str(x_left[i]), ',', str(x_right[i]), ']'
                print ' '
                show()
...
# posteriors for model Bt
            ...
            for i in range(numberOfRuns):
                if cmp(report,'Nothing')!=0:
                    print "Run #"+str(i+1) + ": " + str(FF[i]) + "|" + str(Res[i,:])
...
# compute posteriors
        ...
        for i in range(post.shape[0]):
            ffile.write(array_format(originaldata[i,:],' %d,') + '|'+array_format(post[i,:], '%5.4f,')+'\n')
            if cmp(report,'Everything')==0:
                ...
                print string_wrap(s1,1) + "|" + string_wrap(s2,2) + "|" + string_wrap(s3,3)
                    
        if cmp(report,'Everything')==0:
            print string_wrap('*Data* | *Posteriors* for all annotation values [1, 2, ...N_max] | *thetas* for the three evaluators',1)
...
if __name__ == '__main__':
    ...
    main_view = View(Group(Item(name='input_data'),
                           Item(name='optional_labels',label='Optional file with labels for codes'),
                           ...
                           Item(name = '_'),
                           Item(name='estimate_variances',enabled_when="input_data!=None"),
                           Item(name='Metropolis_jumps', label='Number of Metropolis-Hastings jumps',
                           ...
                           Item(name='evaluation_jumps',enabled_when="input_data!=None and estimate_variances==True"),
                           Item(name='recomputing_cycle',enabled_when="input_data!=None and estimate_variances==True"),
                           Item(name='per_cent_delta',
                                label='Maximum deviation from the target rejection rate (%)',
                                enabled_when="input_data!=None and estimate_variances==True"),                           
                           Item(name='significance',enabled_when="input_data!=None and estimate_variances==True"),
                           Item(name='figure_dpi',
                                label='Resolution of figures (dpi)',
                                enabled_when="input_data!=None and estimate_variances==True"),
                           Item(name = '_'),
                           Item(name='run_computation_now',enabled_when="input_data!=None",show_label=False),
